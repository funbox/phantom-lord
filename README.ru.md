...
В корне проекта находится декларационный файл `index.d.ts`. Он помогает IDE подсвечивать поля и методы класса RemoteBrowser и содержит информацию о аргументах методов и возвращаемом значении.
Если в процессе разработки добавляются новые команды, удаляются старые, или как-либо изменяется интерфейс класса, рекомендуется соответствующим образом отредактировать декларационный файл.
Для подстраховки добавлены тесты, которые проверяют соответствие методов, объявленных в декларационном файле, командам из `lib/commands` и собственным методам `RemoteBrowser`.
...
В случае, если команда `sendKeys` используется для заполнения поля с маской, необходимо передать третий параметр `caretPosition` со значением `start`.

Пример:
```javascript
await browser.sendKeys('.text-field_masked input[type=text]', '9261112233', 'start');
```

Причина: если у поля есть маска, то при фокусе у него автоматически появляется атрибут `value` со значением в виде пустой маски (например, `value="___ ___-__-__"`). По-умолчанию, параметр `caretPosition` имеет значение `end`, таким образом, курсор ставится после `___ ___-__-__`, и переданное значение не вводится, либо вводится некорректно.
...
```
npm install -D @funboxteam/phantom-lord
```
