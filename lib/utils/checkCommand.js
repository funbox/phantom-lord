...
const allowedBeforeBrowserStart = ['addStubToQueue', 'addCookieToQueue', 'addLocalStorageItemToQueue', 'addTestSetting'];
const allowedBeforeInitialization = [...allowedBeforeBrowserStart, 'open'];
...
const checkCmd = function checkCommand(context, cmdName, ...cmdArgs) {
  ...
  if (context.state !== 'started' && !allowedBeforeBrowserStart.includes(cmdName)) {
