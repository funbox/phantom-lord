/* eslint-disable import/no-extraneous-dependencies, import/no-unresolved, no-undef, no-underscore-dangle, no-var, prefer-arrow-callback, prefer-template, object-shorthand, prefer-spread, vars-on-top, func-names */
...
while (!listening) {
  listening = server.listen(port, processRequest);
...
function processCmd(cmd, response) {
  ...
  var commands = {
    ...
    fillForm: function () {
      ...
      var fillResults = page.evaluate(function (selector_, vals_, selectorType_) {
        try {
          return __utils__.fill(selector_, vals_, selectorType_);
        } catch (exception) {
      ...
      // Form submission?
      if (submit) {
        page.evaluate(function _evaluate(selector_) {
          var form = __utils__.findOne(selector_);
